...
def calculate_values_with_formula(table_data,f,u):
    # Cette fonction va remplir les cellules contenant des zéros avec la formule spécifiée
    
    for film in range(1, f):
        for user in range(1, u):
            ...
            # Conserver la couleur bleue si la valeur initiale de la cellule était non nulle
            # if was_non_zero:
            #     table_data.loc[film, user] = '<span style="color: blue;">' + str(table_data.loc[film, user]) + '</span>'
                if was_non_zero:
                    table_data.loc[film, user] = '<span style="color: blue;">' + str(table_data.loc[film, user]) + '</span>'
...
def main():
    st.title("Remplissage d'un tableau de films par utilisateur")
    ...
    if st.button("Calculer"):
        ...
        table_data = calculate_values_with_formula(table_data,num_movies+1,num_users+1)
    
    # Appliquer la mise en forme pour les cellules contenant des zéros
    table_style = table_data.style.applymap(lambda x: 'background-color: white' if x == 0 else '')
    
    # Appliquer le format de deux chiffres après la virgule à toutes les valeurs du tableau
    table_style = table_style.format("{:.2f}")

    # Afficher le tableau
    st.write(table_style)
